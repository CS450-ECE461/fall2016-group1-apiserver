...
function ResourceController (options) {
  ...
  _.each(this.sensitivePaths, function (sensitivePath) {
    self.defaultProjection[sensitivePath] = false;
  });
}
...
ResourceController.prototype.task = function (operation) {
  ...
  return function __ResourceController_executeTask (request, response, next) { // eslint-disable-line camelcase
    ...
    }, function __ResourceController_handleTaskError (error) { // eslint-disable-line camelcase
      if (error) {
        return next(error);
      }
...
ResourceController.prototype._create = function (request, response, next) {
  ...
  this.model.create(request.body[this.singular], function (error, result) {
    ...
    self.findOneByUnique(result._id, function (error, result) {
      if (error) {
        return next(errors.normalizeError(error));
      }
