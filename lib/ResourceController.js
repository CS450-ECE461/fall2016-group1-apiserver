...
function ResourceController(options) {
    ...
    this.includeVersionKey = options.includeVersionKey || false;

    if (!this.includeVersionKey && this.model.schema.versionKey) {
        if (this.models.schema.versionKey == true) {
            this.sensitivePaths.push('__v');
        } else {
            this.sensitivePaths.push(this.model.schema.versionKey);
        }
    }

    this.defaultProjection = {};

    _.each(this.sensitivePaths, function(sensitivePath) {
        self.defaultProjection[sensitivePath] = 0;
    });

    console.log("sensitive paths for " + this.model.modelName + ": " + JSON.stringify(this.sensitivePaths));
    console.log("default projection for " + this.model.modelName + ": " + JSON.stringify(this.defaultProjection));
}
...
ResourceController.prototype._create = function (request, response, next) {
    ...
    this.model.create(request.body[this.singular], function (error, result) {
        ...
        self.findOneByUnique(result._id, this.defaultProjection, function (error, result) {
            if (error) {
                return next(errors.normalizeError(error));
            }
...
ResourceController.prototype._get = function (request, response, next) {
    var self = this;
    this.findOneByUnique(request.params.id, this.defaultProjection, function (error, result) {
        if (error) {
            return next(error);
        }
...
ResourceController.prototype._getAll = function (request, response, next) {
    ...
    for (var i = 0; i < this.sensitivePaths.length; i++) {
    ...
    this.model.count(request.query, function (error, count) {
        if (error) {
            return next(errors.normalizeError(error));
        }

        self.model.find(conditions, this.defaultProjection, options, function (error, results) {
            if (error) {
                return next(errors.normalizeError(error));
            }
